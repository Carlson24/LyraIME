...
class ScreenshotClipboardWatcher(
    ...
    companion object {
        private const val TAG = "LyraScreenshotWatcher"
        ...
        return runCatching {
            context.contentResolver.query(
                ...
                while (cursor.moveToNext() && checked < QUERY_LIMIT) {
                    ...
                    if (!looksLikeScreenshot(displayName, bucket, relativePath)) continue
    ...
    private fun copyScreenshotToClipboard(candidate: ScreenshotCandidate) {
        ...
        runCatching {
            ...
            scope.launch {
                ClipboardHelper.importLocalEntry(
                    text = uri.toString(),
                    type = mimeType,
    ...
    private fun screenshotKey(
        ...
        displayName: String,
        dateAdded: Long,
    ): String = "$mediaId|$displayName|$dateAdded"
