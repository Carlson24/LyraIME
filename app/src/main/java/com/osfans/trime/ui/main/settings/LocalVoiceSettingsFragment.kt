...
class LocalVoiceSettingsFragment : PreferenceDelegateFragment(AppPrefs.defaultInstance().localVoice) {
    ...
    private val prefsListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        if (key == AppPrefs.LocalVoice.VOICE_MODEL_TYPE) {
            refreshModelUi()
            refreshDspUi()
        }
    }
    ...
    private fun refreshDspUi() {
        val selected = AppPrefs.defaultInstance().localVoice.voiceModelType.getValue()
        val isQnn = selected == AppPrefs.LocalVoice.VoiceModelType.QNN
        dspInitPref?.isEnabled = isQnn
        if (isQnn) {
            val installed = QnnDspManager.isInstalled()
            dspInitPref?.title = getString(
                if (installed) R.string.voice_qnn_dsp_reinitialize
                else R.string.voice_qnn_dsp_initialize,
            )
            dspInitPref?.summary = getString(
                if (installed) R.string.voice_qnn_dsp_installed
                else R.string.voice_qnn_dsp_not_installed,
            )
        } else {
    ...
    private fun startDspDownload() {
        ...
        dspJob = lifecycleScope.launch {
            try {
                ...
                val status = if (result != null) getString(R.string.voice_qnn_dsp_installed)
                else getString(R.string.voice_qnn_dsp_download_failed)
    ...
    private fun refreshModelUi() {
        ...
        downloadPref?.title = getString(
            if (installed) R.string.voice_model_download_action_reinstall
            else R.string.voice_model_download_action,
        )
        downloadPref?.summary = getString(
            if (installed) R.string.voice_model_status_installed
            else R.string.voice_model_status_not_installed,
            variantName,
        )
    ...
    private fun startDownload() {
        ...
        downloadJob = lifecycleScope.launch {
            try {
                        ...
                        onProgress = { progress, downloaded, total ->
                            launch(Dispatchers.Main) {
                                ...
                                taskText.text = getString(
                                    R.string.wanxiang_dl_progress_line, taskTitle, status,
                                )
                                if (progress < 0f) {
                ...
                taskText.text = getString(
                    R.string.wanxiang_dl_progress_line, taskTitle,
